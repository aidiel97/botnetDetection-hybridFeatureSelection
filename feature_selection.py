...
def exportLogFeatureSelection(dataset, name, feature):
    ...
    dataset = dataset.replace(CTU_DIR,"ctu")
    dataset = dataset.replace(NCC_DIR,"ncc")
    dataset = dataset.replace(NCC_2_DIR,"ncc-2")
    dataset = dataset.replace(UNSW_NB15_DIR,"unsw-nb15")
